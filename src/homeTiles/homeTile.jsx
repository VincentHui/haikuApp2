...
const TileWithSpring = ({ onClick, children }) => {
  ...
  return (
    <TileButton
      {...inputFunctions}
      style={{ width: "100%", transform: transform }}
    >
      {children}
    </TileButton>
  );
...
const InitialTile = ({
  ...
  const [MouseScale, setMouseScale] = useState(1.0);
  const inputFunctions = titlePress(
    () => toogleFlipped(!flipped, contentKey),
    () => setMouseScale(0.7),
    () => {
      setMouseScale(1.0);
      toggleHover(false);
  ...
  const { transform, opacity, TileHeight } = useSpring({
    opacity: flipped ? 1 : 0,
    transform: `perspective(600px) rotateY(${flipped ? 180 : 0}deg) scale(${
      MouseScale + (flipped ? 0.1 : -0.1) + (hover ? 0.1 : 0.0)
    })`,
  ...
  return (
    ...
    >
      <TileContainer>
        <HomeTile
          style={{
            height: TileHeight.interpolate((h) => TILE_HEIGHT - h),
...
const mapStateToProps = (state, props) => ({
  tile: state.homeTiles.filter(
    (tile) => tile.contentKey === props.contentKey
  )[0],
  Selected: state.SelectedTile,
  flipped: state.homeTiles.filter(
    (tile) => tile.contentKey === props.contentKey
  )[0].flipped,
});
